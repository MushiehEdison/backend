            ...
            try:
                ...
                if not ai_response_text or not isinstance(ai_response_text, str) or ai_response_text.strip() == "":
                    ...
                    if is_mic_input:
                        ...
                        logger.info(f"Generating TTS for language: {language}, text: {ai_response_text[:50]}...")
                        audio_base64, audio_error = generate_tts_audio(ai_response_text, user.id, conversation.id, language)
                        if not audio_base64 and not audio_error:
                            logger.warning("No audio or error returned from TTS")
                            audio_error = "No audio generated by TTS service"
